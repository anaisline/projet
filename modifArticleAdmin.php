...
if ($db_found) {
	...
	if($nom!="")
	{
		$sql="UPDATE article_admin SET nom='$nom' WHERE id_article='$id_articleAModif' ";
		$result = mysqli_query($db_handle,$sql);
	}
	if($description!="")
	{
		$sql="UPDATE article_admin SET description='$description' WHERE id_article='$id_articleAModif' ";
		$result = mysqli_query($db_handle,$sql);
	}
	if($prixArticle!="")
	{
		$sql="UPDATE article_admin SET prix='$prixArticle' WHERE id_article='$id_articleAModif' ";
		$result = mysqli_query($db_handle,$sql);
	}
	if($photo1!="")
	{
	...
	if($typepaiement!="")
	{
		...
		{
			$sql="UPDATE article_admin SET categorie_achat='$typepaiement' WHERE id_article='$id_articleAModif' ";
			$result = mysqli_query($db_handle,$sql);	
		}
	...
	if($erreur==0)
	{
		header('Location: pageModifArticleAdmin.php?erreur=0');
	}
	else
	{
		header('Location: pageModifArticleAdmin.php?erreur=1');
	}
